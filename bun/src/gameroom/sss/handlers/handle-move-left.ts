...
export function handle_move_left(ws: Bun.ServerWebSocket<WebSocketData>, msg: Uint8Array):GameRoomResponseMessage[] {
  ...
  b.update_one_ship_rotations(uuid, now) // warning todo check properly. It updates all but need only one uuid based, or just nothing. Possibly multifail, for all cases except -move-stop and -move-front
  b.set_avf(uuid, 0)
  b.set_avs(uuid, 0)
