...
export function handle_shot_right(ws: Bun.ServerWebSocket<WebSocketData>, msg: Uint8Array):GameRoomResponseMessage[] {
  devlog("handle_shot_right() execution.")
  ...
  try {
    ...
    power = obj.power/100
    if(!power){
      errlog("incorrect right shot message from client(no obj.power)")
      return result
    } else if (power < 0 || power > 1){
      errlog("right shot power outside of allowed range. Hijacking")
      ws.close(CCR.HIJACKING.code, CCR.HIJACKING.reason)
    }
  } catch (e) {
    errlog("incorrect right shot message from client","mm.u8aobj(msg) parsing fail")
    return result
  }
  ...
  if (!a) return [{ mt: MT.S, msg: { alert_text:"your ship does not have right gun, ... buddy", }, ms: 0, uuids: [uuid] }]
  ...
  const sv = gemm.vec3Dnormal([s.fvx,s.fvy,s.fvz],[s.tvx,s.tvy,s.tvz]) // left/right different
  let vx = sv[0]!
  let vy = sv[1]!
  let vz = sv[2]!
